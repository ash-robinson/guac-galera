...
if [ -n "$mysqlpwd" ] && [ -n "$guacuser" ] && [ -n "$guacpwd" ] && [ -n "$host" ] ; then
    ...
    while true
    do
        read -s -p "Enter a remote host: " remotehost
        echo
        read -s -p "Confirm remote host: " password2
        echo
        [ "$remotehost" = "$password2" ] && break
...
create user '$guacdbusername'@'$remotehost' identified by \"$guacdbuserpassword\";
GRANT SELECT,INSERT,UPDATE,DELETE ON guacamole_db.* TO '$guacdbusername'@'$remotehost';
flush privileges;"
...
echo "guacamole database installed successfully, access configured for ${guacdbusername} from ${remotehost}"